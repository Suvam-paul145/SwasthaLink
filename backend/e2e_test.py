...
def main():
    ...
    doc_id = doc["user"]["id"]
    print(f"[1] Doctor logged in: {doc_id}")
    ...
    pat_id = pat["user"]["id"]
    print(f"[2] Patient logged in: {pat_id}")
    ...
    adm_id = adm["user"]["id"]
    print(f"[3] Admin logged in: {adm_id}")
    ...
    print(f"[4] Upload status: {resp.status_code}")
    ...
    print(f"[6] Approve status: {approve_resp.status_code}")
    ...
    print(f"[7] Prescriptions for {test_pid}: {rx_for_pid['count']}")
    ...
    print(f"[8] Patient profile: linked_pid = {profile.get('linked_pid')}")
    ...
    print(f"[9] Link PID status: {link_resp.status_code}")
    link_result = link_resp.json()
    print(f"    Result: {link_result}")
    ...
    print(f"[10] Patient profile after link: linked_pid = {profile2.get('linked_pid')}")

    # 11. Prescriptions for patient UUID (should be empty — prescriptions stored under PID)
    rx_uuid = requests.get(f"{BASE}/api/prescriptions/for-patient/{pat_id}").json()
    print(f"[11] Prescriptions for patient UUID ({pat_id}): {rx_uuid['count']}")
    ...
    print(f"[12] Prescriptions for linked PID ({test_pid}): {rx_pid['count']}")

    # Summary
    print("\n--- SUMMARY ---")
    print(f"Patient UUID (system-generated at signup): {pat_id}")
    print(f"Prescription PID (system-generated at upload): {test_pid}")
    print(f"Prescription uploaded: {'YES' if rx_id else 'NO'}")
    print(f"Prescription approved: {'YES' if rx_for_pid['count'] > 0 else 'NO'}")
    print(f"PID linked to patient: {'YES' if profile2.get('linked_pid') == test_pid else 'NO'}")
    print(f"Patient can see prescriptions via PID: {'YES' if rx_pid['count'] > 0 else 'NO'}")
    ...
    print(f"\nAll flows working: {'YES ✓' if all_ok else 'NO ✗'}")
